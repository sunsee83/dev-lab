...
이 문서는 통합 북마클릿의 **데이터 추출 단계**에서 코어와 공개 UI가 주고받는 결과 형식을 정리합니다.
...
`ui.html`은 `YT_TOOL_DATA_RESULT`를 받으면:

- 현재 실행 메모리에만 결과 보관
- 브라우저 저장소에 자동 저장하지 않음
- 새 영상 초기화 시 이전 결과 제거
- 데이터 선택/요청 조건이 바뀌면 이전 결과를 폐기
- 단계 7의 `원문 / TXT / JSON` 출력에 넘길 수 있도록 `getDataResult()` 제공

## 단계 6 완료 기준

- UI가 선택 필드와 댓글 옵션을 코어에 전달
- 코어가 선택된 필드만 조사
- 부분 실패 허용
- `YT_TOOL_DATA_RESULT` 결과 형식 확정
- UI가 결과를 실행 메모리에만 수신/보관
