...
- 위치: 카드 좌측 상단
- 크기: 약 28px
- 아이콘: 다운로드/미디어 액션 의미. 플레이 모양 금지
- Instagram 기본 우측 상단 미디어 아이콘은 그대로 둠
- 카드 자체 클릭은 원래 Instagram 게시물 이동을 유지

버튼을 누르면 현재 카드 shortcode에 연결된 작은 메뉴를 표시합니다.

REEL / VIDEO:
- `영상 열기` — 직접 video URL이 검증된 경우
- `썸네일 열기`
- `링크 복사`
...
- `대표 이미지`
- `링크 복사`
- 전체 슬라이드 액션은 `media[]` 수집이 구현된 뒤 추가

규칙:

- 검증되지 않은 영상/슬라이드 URL을 만들어내지 않음
- video URL 미확보 시 `영상 준비중`처럼 비활성 상태로 표시 가능
- 메뉴 조작 시 카드 본문 클릭을 발생시키지 않음
- 카드 밖 탭/스크롤/리사이즈/페이지 이동 시 메뉴 닫기
- React가 카드 DOM을 재사용하면 현재 href shortcode 기준으로 메뉴를 다시 연결
