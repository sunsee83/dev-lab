...
3. 전체 구조에 미치는 영향 재검토
4. 관련 기준 문서 갱신
5. `WORK_TRACK.md`의 현재 작업과 다음 순서를 갱신
6. 그 다음 코드 수정
...
현재 목표는 **기존 v3.1에서 실기기로 확인된 Grid/미디어 개선을 유지하면서, v3.2의 공용 RI/Data 구조를 단일 owner 체계로 전환하는 것**입니다.

현재 집중 범위:

1. 남아 있는 신규 UI 중복코드 제거
2. Reel current identity 정확도 개선
3. Reel native likes/comments/reposts 수집 정확도 개선
4. Reel overlay와 RI Summary가 동일 Metrics Engine 사용
5. 기존 legacy metric 계산 함수 제거
6. 이후 Identity → Extractor → Verified Store 순으로 Data Engine migration
...
다음 코드 작업의 첫 번째 항목으로 **작은 공용 RI UI primitive를 만들고 warning을 0으로 줄입니다.** 단, 의미 없는 거대한 `utils.js`는 만들지 않습니다.
